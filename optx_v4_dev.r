...
source("https://raw.githubusercontent.com/pjerrot/r_stuff/master/giniauc.R")
...
opt.x <- function(crittypen="gini",targetdef,df,db="SQLite", includetree=TRUE, includeinteractions=TRUE,plots=FALSE) {
    ...
    tryCatch(
      {
        ...
        } else if (crittypen=="gini"){
          tmp$predicts <- predict(tmp_mod,newdata=tmp)
          critvaluen <- giniauc(tmp,"predicts","binarytarget")["gini"]
        } else if (crittypen=="auc") {
          tmp$predicts <- predict(tmp_mod,newdata=tmp)
          critvaluen <- giniauc(tmp,"predicts","binarytarget")["auc"]
        }
    ...
    if (is.numeric(df[,i])) {
      ...
      if (max(is.na(df[,i])) == 1) {
        ...
        if (crittypen=="r2") {
          ...
          test$predicts <- predict(model,newdata=test)
          critvaluen <- giniauc(test,"predicts","binarytarget")["gini"]
        } else if (crittypen=="auc") {
          test$predicts <- predict(model,newdata=test)
          critvaluen <- giniauc(test,"predicts","binarytarget")["auc"]
        }
      ...
      if (crittypen=="r2") {
        ...
        critvaluen <- giniauc(df,i,"binarytarget")["gini"]
      } else if (crittypen=="auc") {
        critvaluen <- giniauc(df,i,"binarytarget")["auc"]
      }
      ...
      if (tabel2[i]>100) {
        ...
        if ((is.na(test[1,1])==FALSE)&(is.na(test[1,2])==FALSE))
        {
  ...
  tryCatch(
    {
      ...
      fit = cv.glmnet(x=inputmatrix, y=targetvector, type.measure='auc',nfolds=15,alpha=.5, family = "binomial",standardize = TRUE) # <- elastic net
