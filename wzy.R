...
.replace_ae_oe_aa <- function(str) {
  str <- gsub("Ã¦", "&aelig;", str)
  str <- gsub("Ã¸", "&oslash;", str)
  str <- gsub("Ã¥", "&aring;", str)
  str <- gsub("Ã<U+0086>", "&AElig;", str)
  str <- gsub("Ã<U+0098>", "&Oslash;", str)
  str <- gsub("Ã<U+0085>", "&Aring;", str)
  return(str)
}
...
wz.title <- function(str, size = c("h3", "h1", "h2", "h4", "h5", "h6"),
  ...
  if (is.null(link)) {
    ...
    .wcontent <<- c(.wcontent, paste0(
      tmp, "<", size, "><a name='", a_name, "' href='", link, "' style='text-decoration:none;'>", ifelse(small == TRUE, "<small>", ""), "<font color='", color, "'><br>",
...
wz.linebreak <- function(count = 1) {
  for (i in 1:count) {
    .wcontent <<- c(.wcontent, paste0("<br>"))
  }
}

# insert.html ####
wz.html <- function(str) {
  .wcontent <<- c(.wcontent, str)
}
...
wz.text <- function(str, fontsize = 16, fontface = "Arial",
  ...
  if (scrollbar == TRUE) {
    htmp <- paste0(
      "<table align='", align, "' width=", boxwidth, " cellpadding=", .cellpadding, "><tr><td width=",
  ...
  if (length(.wcontent) > 0) {
    for (i in 1:length(.wcontent)) {
      ...
      if (grepl("HTMLTABLE_", .wcontent[i])) {
        ...
        table_id <- substr(
          .wcontent[i], as.numeric(gregexpr(pattern = "id=", .wcontent[i])[[1]]) + 3,
          as.numeric(gregexpr(pattern = "align", .wcontent[i])[[1]]) - 2
        )
        for (j in 1:length(.wcontent)) {
          if (grepl("INSERT2HTMLTABLE", .wcontent[j]) &
            substr(
              .wcontent[j], as.numeric(gregexpr(pattern = "id=", .wcontent[j])[[1]]) + 3,
              as.numeric(gregexpr(pattern = "cell_id", .wcontent[j])[[1]]) - 2
            ) == table_id) {
            cell_id <- substr(
              .wcontent[j], as.numeric(gregexpr(pattern = "cell_id=", .wcontent[j])[[1]]) + 8,
...
wz.columnchart <- function(df, x, group_var = NULL, num_vars, fun = c("asis", "sum", "mean", "median", "sd"),
  ...
  htmp <- paste0(htmp, "var options = {\n")
  htmp <- paste0(htmp, paste0("title: '", ifelse(!is.null(chart_title), chart_title, paste0(
    "Column chart of ", paste(num_vars, collapse = " and "), " - by ", x,
    ifelse(!is.null(group_vars), paste0(" - grouped by ", group_var), "")
  )), "',\n"))
...
wz.piechart <- function(df, group_var, num_var = NULL, fun = c("asis", "n", "sum", "mean", "median", "sd"),
  ...
  if (!is.null(num_var)) {
    if (fun == "asis") {
      ...
      df2 <- df %>%
        ...
        summarise_at(.vars = num_var, .funs = fun)
    }
  } else {
    num_var <- "n"
    df2 <- df %>%
  ...
  for (i in 1:nrow(df2)) {
    htmp <- paste0(htmp, paste0(
      "['", as.character(df2[i, group_var]), "',", as.character(paste(df2[i, num_var], collapse = ",")),
      ifelse(!is.null(annotation_var), paste0(",'", as.character(df2[i, annotation_var]), "'"), ""), "]", ifelse(i == nrow(df2), "", ","), "\n"
    ))
  }
...
wz.linechart <- function(df, x = NULL,
  ...
  if (is.null(group_var)) {
    if (!is.null(num_vars)) {
      if (fun == "asis") {
        ...
        if (fun == "n") {
          ...
          df2 <- df %>%
            group_by(.data[[x]]) %>%
            summarise_at(.vars = num_vars, .funs = fun)
          df2 <- data.frame(df2)
        }
        df2 <- df2[order(df2[, x]), ]
      }
      ...
      if (fun == "sum") {
        ...
        df2 <- df %>%
          group_by(.data[[x]]) %>%
          summarise_at(.vars = num_vars, .funs = "sum")
        df2 <- data.frame(df2)
        df2 <- df2[order(df2[, x]), ]
      } else if (fun == "acc_n") {
        num_vars <- "acc_n"
        df2 <- df %>%
          group_by(.data[[x]]) %>%
          summarise_at(.vars = num_vars, .funs = "cumsum")
        df2 <- data.frame(df2)
    ...
    if (!is.null(num_vars)) {
      ...
      if (fun == "acc_n") {
        ...
        df2 <- df2 %>%
          group_by(.data[[group_var]]) %>%
          summarise(x = .data[[x]], n = n, acc_n = cumsum(n))
        df2 <- data.frame(df2)
  ...
  tilf <- runif(nrow(df2))
  if (nrow(df2) > samplesize & samplesize>0) {
  ...
  for (i in 1:nrow(df2)) {
    ...
    htmp <- paste0(htmp, paste0(
      "[", insx, ",", as.character(paste(df2[i, num_vars], collapse = ",")),
      ifelse(!is.null(annotation_var), paste0(",'", as.character(df2[i, annotation_var]), "'"), ""), "]", ifelse(i == nrow(df2), "", ","), "\n"
    ))
  }
  ...
  htmp <- paste0(htmp, "colors: [", .colorstr, "],\n")
  if (smooth == TRUE) {
    htmp <- paste0(htmp, "curveType: 'function',\n")
  }
  htmp <- paste0(htmp, "titleTextStyle: {fontSize: ", titlefontsize, "},\n")
  if (length(num_vars) == 1) {
    htmp <- paste0(htmp, "vAxis: {title: '", gsub("asis ", "", paste(fun, num_vars[1])), "'},\n")
  }
...
wz.histogram <- function(df, num_var, breaks = 10,
  ...
  if (is.null(chart_title)) {
    chart_title <- paste0("Histogram of ", num_var, ifelse(log == TRUE, " (log)", ""))
  }
  ...
  if (log == TRUE) {
    if (any(na.omit(df[, num_var]) <= 0) == TRUE) {
      warning(paste0("Rows with ", num_var, "<=0 have been removed!"))
    }
    df <- df[df[, num_var] > 0, ]
  ...
  tmp <- df %>%
    group_by(cat) %>%
    summarise(n = n(), min_value = min(get(num_var)), max_value = max(get(num_var)))
  ...
  wz.columnchart(tmp,
    group_var = xname, num_vars = c("n"), chart_title = chart_title,
...
wz.image <- function(image, align = "center", width = NULL, height = NULL) {
  ...
  if (grepl("http", image)) {
    htmp <- paste0(
      "<table align='", align, "' border=0 width=800 cellpadding=", .cellpadding,
      "><tr><td align='", align, "'>
